...
def process():
    ...
    try:
        ...
        for label, orig in clean_to_orig.items():
            ...
            trace = {"x": ts_list, "y": arr, "mode": "lines", "name": label, "type": "scatter"}
            if is_y2(label):
                trace["yaxis"] = "y2"
            traces.append(trace)

        # layout: legend at top; unified hover; add y2 if needed
        layout = {
            "hovermode": "x unified",
            "showlegend": True,
        ...
        # Use single braces here (correct for Plotly); we inject via JSON safely
